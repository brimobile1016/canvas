...
module.exports = async function card(reqQuery) {
	...
	for (const prop of Object.entries(defaults)) {
		if (typeof query[prop[0]] != 'string' || !query[prop[0]].length)
			query[prop[0]] = prop[1];
	}

	let { avatar, middle, name, bottom, text, avatarborder, avatarbg, background } = query;

	const none = [{ avatar }, { middle }, { name }, { bottom }].find(
		(que) => typeof Object.values(que)[0] != 'string' || !Object.values(que)[0].length
	);
	...
	const canvas = createCanvas(768, 375);
	...
	const buffer = await canvas.encode('png'); // return Uint8Array
...
module.exports.usage =
	'?avatar=url&middle=text&name=text&bottom=text[&text=hex|rgba&avatarborder=hex|rgba&avatarbg=hex|rgba&background=url|hex|rgba]';
